{
 ...
 T->Add("output/Phasespaceactor.root");
 ...
 histoEmittanceXTheta = new TH2D("histoEmittanceXTheta", "Emittance X #theta",100, -5, 5, 300, -1500, 1500);

 TH2D *histoEmittanceYPhi;
 histoEmittanceYPhi = new TH2D("histoEmittanceYPhi", "Emittance Y #phi",100, -5, 5, 100, -40, 40);

 TH2D *histoSpotSize;
 histoSpotSize = new TH2D("histoSpotSize", "Spot",100, -5, 5, 100, -5, 5);

 TH1D *histoEnergy;
 histoEnergy = new TH1D("histoEnergy", "Energy",100, 0, 0.7);
...
for (int i=0; i<n; i++)
{   T->GetEntry(i);
  histoEmittanceXTheta->Fill(x,atan(dx/dz)*100,weight); // x-axis in mm and y-axis in mrad
  histoEmittanceYPhi->Fill(y,atan(dy/dz)*100,weight);
...
fenetre1->cd(3);
histoSpotSize->Draw("surf2");
fenetre1->cd(4);
histoEnergy->Draw("col");
}
